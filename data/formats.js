...
exports.BattleFormats = {
	...
	abilityexchangepokemon: {
		...
		validateTeam: function(team, format) {
			var selectedAbilities = [];
			var defaultAbilities = [];
			var bannedAbilities = {adaptability:1, arenatrap:1, contrary:1, hugepower:1, imposter:1, purepower:1, prankster:1, serenegrace:1, shadowtag:1, simple:1, tintedlens:1, wonderguard:1};
			var problems = [];
			for (var i=0; i<team.length; i++) {
				var template = this.getTemplate(team[i].species);
				var ability = this.getAbility(team[i].ability);
			...
			if (problems.length) return problems;
			if (!this.checkAbilities(selectedAbilities, defaultAbilities)) {
				return ['That is not a valid Mixedmons team.'];
			}
		}
	},
		metronomeclause: {
